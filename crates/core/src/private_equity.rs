...
pub fn m_pme_2(
  ...
  let d_weight: Vec<_> = distributions
    .iter()
    .zip(nav)
    .map(|(d, n)| d / (d + n))
    .collect();
  ...
  for t in 1..nav_pme.len() {
    nav_pme[t] = (1. - d_weight[t]) * (nav_pme[t - 1] * index[t] / index[t - 1] + contributions[t]);
  }
...
fn check_input_len(amounts: &[f64], index: &[f64]) -> Result<()> {
  if amounts.len() != index.len() {
    Err(InvalidPaymentsError::new(
      "Amounts must be the same length as index.",
    ))
  } else if index.is_empty() {
    Err(InvalidPaymentsError::new(
      "Input array must contain at least one value",
    ))
  } else {
    Ok(())
  }
...
fn split_amounts(amounts: &[f64]) -> (Vec<f64>, Vec<f64>) {
  ...
  let contributions: Vec<_> = amounts
    .iter()
    .map(|x| x.clamp(f64::MIN, 0.).abs())
    .collect();
